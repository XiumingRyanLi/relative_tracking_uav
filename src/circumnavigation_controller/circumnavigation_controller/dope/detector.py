# Vendored from NVIDIA Deep_Object_Pose/common (CC BY-NC-SA 4.0).
# Local changes: package-relative imports, pyrr dependency removed,
# per-frame prints silenced, torchvision/scipy API updates.
...
class ObjectDetector(object):
    ...
    def find_object_poses(
        ...
        for obj in objects:
            ...
            # Local change: the stock code drops a cuboid if ANY corner is
            # missing. Keep it whenever PnP is solvable (>=4 corners + the
            # centre) and report how many corners were found, so the caller
            # can decide (dope_detector accepts partial cuboids only while
            # tracking and only if they agree with the current track).
            n_corners = sum(1 for p_ in points[:8] if p_ is not None)
            if n_corners < 4:
                continue

            cuboid2d = list(points)  # may contain None for missing corners; np.copy would raise
    ...
    def find_objects(
        ...
        for j in range(vertex2.size()[0]):
            ...
            # Local change: score each peak by the RAW belief maximum in a
            # 3x3 window around the smoothed-map peak, not the single raw
            # pixel under it. For small objects (a car ~100 px wide gives
            # belief blobs only 1-2 cells wide at 1/8 resolution) the
            # sigma=3 smoothing shifts the peak by a cell, where the raw
            # value can be ~0.01 and the object was silently dropped.
            def _win_max(px, py):
                y0, y1 = max(0, py - 1), min(map_ori.shape[0], py + 2)
                x0, x1 = max(0, px - 1), min(map_ori.shape[1], px + 2)
                return float(map_ori[y0:y1, x0:x1].max())
            peaks_with_score = [
                peaks_avg[x_] + (_win_max(peaks[x_][0], peaks[x_][1]),)
