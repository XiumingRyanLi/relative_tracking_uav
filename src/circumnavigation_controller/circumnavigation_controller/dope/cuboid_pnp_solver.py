...
class CuboidPNPSolver(object):
        ...
        if is_points_valid:
            ...
            if ret:
                ...
                if z < 0:
                    ...
                    rotate_angle = np.pi
                    rotate_quaternion = tf_transformations.quaternion_about_axis(
                        rotate_angle, location
                    )
                    quaternion = list(
                        tf_transformations.quaternion_multiply(rotate_quaternion, quaternion)
                    )
    ...
    def convert_rvec_to_quaternion(self, rvec):
        ...
        T = np.eye(4)
        T[:3, :3] = R
        # Order is XYZW, matching the pyrr quaternion this replaces.
        return list(tf_transformations.quaternion_from_matrix(T))
